        ...
        for sample in 0..stream_info.buffer_size {
            for channel in 0..stream_info.channels {
                if self.sample_clock < attack {
                    // attack phase
                    buffer.set_sample(channel, sample, self.sample_clock / attack);
                ...
                {
                    // sustain phase
                    buffer.set_sample(channel, sample, sustain_level);
                    ...
                    && (self.sample_clock - (total - release) < release)
                {
                    ...
                    buffer.set_sample(channel, sample, sustain_level * level);
                } else {
                    // after release
                    buffer.set_sample(channel, sample, 0.0);
                }
            }
